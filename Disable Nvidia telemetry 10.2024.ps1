@echo off
:: https://privacy.sexy â€ v0.13.6 â€ Fri, 18 Oct 2024 05:24:46 GMT
...
$taskPathPattern=\; $taskNamePattern=NvTmRepOnLogon_{B2FE1952-0186-46C3-BAEC-A80AA35AC5B8}; Write-Output ^Disabling tasks matching pattern `^$taskNamePattern`^.^; $tasks = @(Get-ScheduledTask -TaskPath $taskPathPattern -TaskName $taskNamePattern -ErrorAction Ignore); if (-Not $tasks) { Write-Output ^Skipping, no tasks matching pattern `^$taskNamePattern`^ found, no action needed.^; exit 0; }; $operationFailed = $false; foreach ($task in $tasks) { $taskName = $task.TaskName; if ($task.State -eq [Microsoft.PowerShell.Cmdletization.GeneratedTypes.ScheduledTask.StateEnum]::Disabled) { Write-Output ^Skipping, task `^$taskName`^ is already disabled, no action needed.^; continue; }; try { $task | Disable-ScheduledTask -ErrorAction Stop | Out-Null; Write-Output ^Successfully disabled task `^$taskName`^.^; } catch { Write-Error ^Failed to disable task `^$taskName`^: $($_.Exception.Message)^; $operationFailed = $true; }; }; if ($operationFailed) { Write-Output Failed to disable some tasks. Check error messages above.; exit 1; }
:: ----------------------------------------------------------
...
$taskPathPattern=\; $taskNamePattern=NvTmMon_{B2FE1952-0186-46C3-BAEC-A80AA35AC5B8}; Write-Output ^Disabling tasks matching pattern `^$taskNamePattern`^.^; $tasks = @(Get-ScheduledTask -TaskPath $taskPathPattern -TaskName $taskNamePattern -ErrorAction Ignore); if (-Not $tasks) { Write-Output ^Skipping, no tasks matching pattern `^$taskNamePattern`^ found, no action needed.^; exit 0; }; $operationFailed = $false; foreach ($task in $tasks) { $taskName = $task.TaskName; if ($task.State -eq [Microsoft.PowerShell.Cmdletization.GeneratedTypes.ScheduledTask.StateEnum]::Disabled) { Write-Output ^Skipping, task `^$taskName`^ is already disabled, no action needed.^; continue; }; try { $task | Disable-ScheduledTask -ErrorAction Stop | Out-Null; Write-Output ^Successfully disabled task `^$taskName`^.^; } catch { Write-Error ^Failed to disable task `^$taskName`^: $($_.Exception.Message)^; $operationFailed = $true; }; }; if ($operationFailed) { Write-Output Failed to disable some tasks. Check error messages above.; exit 1; }
:: ----------------------------------------------------------
...
$pathGlobPattern = ^%SYSTEMROOT%\System32\DriverStore\FileRepository\NvTelemetry*.dll^; $expandedPath = [System.Environment]::ExpandEnvironmentVariables($pathGlobPattern); Write-Host ^Searching for items matching pattern: `^$($expandedPath)`^.^; $renamedCount   = 0; $skippedCount   = 0; $failedCount    = 0; $foundAbsolutePaths = @(); Write-Host Iterating files and directories recursively.; try { $foundAbsolutePaths += @(; Get-ChildItem -Path $expandedPath -Force -Recurse -ErrorAction Stop | Select-Object -ExpandProperty FullName; ); } catch [System.Management.Automation.ItemNotFoundException] { <# Swallow, do not run `Test-Path` before, its unreliable for globs requiring extra permissions #>; }; try { $foundAbsolutePaths += @(; Get-Item -Path $expandedPath -ErrorAction Stop | Select-Object -ExpandProperty FullName; ); } catch [System.Management.Automation.ItemNotFoundException] { <# Swallow, do not run `Test-Path` before, its unreliable for globs requiring extra permissions #>; }; $foundAbsolutePaths = $foundAbsolutePaths | Select-Object -Unique | Sort-Object -Property { $_.Length } -Descending; if (!$foundAbsolutePaths) { Write-Host Skipping, no items available.; exit 0; }; Write-Host ^Initiating processing of $($foundAbsolutePaths.Count) items from `^$expandedPath`^.^; foreach ($path in $foundAbsolutePaths) { if (Test-Path -Path $path -PathType Container) { Write-Host ^Skipping folder (not its contents): `^$path`^.^; $skippedCount++; continue; }; if($revert -eq $true) { if (-not $path.EndsWith(.OLD)) { Write-Host ^Skipping non-backup file: `^$path`^.^; $skippedCount++; continue; }; } else { if ($path.EndsWith(.OLD)) { Write-Host ^Skipping backup file: `^$path`^.^; $skippedCount++; continue; }; }; $originalFilePath = $path; Write-Host ^Processing file: `^$originalFilePath`^.^; if (-Not (Test-Path $originalFilePath)) { Write-Host ^Skipping, file `^$originalFilePath`^ not found.^; $skippedCount++; exit 0; }; if ($revert -eq $true) { $newFilePath = $originalFilePath.Substring(0, $originalFilePath.Length - 4); } else { $newFilePath = ^$($originalFilePath).OLD^; }; try { Move-Item -LiteralPath ^$($originalFilePath)^ -Destination ^$newFilePath^ -Force -ErrorAction Stop; Write-Host ^Successfully processed `^$originalFilePath`^.^; $renamedCount++; } catch { Write-Error ^Failed to rename `^$originalFilePath`^ to `^$newFilePath`^: $($_.Exception.Message)^; $failedCount++; }; }; if (($renamedCount -gt 0) -or ($skippedCount -gt 0)) { Write-Host ^Successfully processed $renamedCount items and skipped $skippedCount items.^; }; if ($failedCount -gt 0) { Write-Warning ^Failed to processed $($failedCount) items.^; }
:: ----------------------------------------------------------
...
$data = 0; reg add HKLM\SOFTWARE\NVIDIA Corporation\NvControlPanel2\Client /v OptInOrOutPreference /t REG_DWORD /d ^$data^ /f
$data = 0; reg add HKLM\SOFTWARE\NVIDIA Corporation\Global\FTS /v EnableRID44231 /t REG_DWORD /d ^$data^ /f
$data = 0; reg add HKLM\SOFTWARE\NVIDIA Corporation\Global\FTS /v EnableRID64640 /t REG_DWORD /d ^$data^ /f
$data = 0; reg add HKLM\SOFTWARE\NVIDIA Corporation\Global\FTS /v EnableRID66610 /t REG_DWORD /d ^$data^ /f
$data = 0; reg add HKLM\SYSTEM\CurrentControlSet\Services\nvlddmkm\Global\Startup /v SendTelemetryData /t REG_DWORD /d ^$data^ /f
:: ----------------------------------------------------------
...
$serviceName = NvTelemetryContainer; Write-Host ^Disabling service: `^$serviceName`^.^; <# -- 1. Skip if service does not exist #>; $service = Get-Service -Name $serviceName -ErrorAction SilentlyContinue; if(!$service) { Write-Host ^Service `^$serviceName`^ could not be not found, no need to disable it.^; Exit 0; }; <# -- 2. Stop if running #>; if ($service.Status -eq [System.ServiceProcess.ServiceControllerStatus]::Running) { Write-Host ^`^$serviceName`^ is running, stopping it.^; try { Stop-Service -Name ^$serviceName^ -Force -ErrorAction Stop; Write-Host ^Stopped `^$serviceName`^ successfully.^; } catch { Write-Warning ^Could not stop `^$serviceName`^, it will be stopped after reboot: $_^; }; } else { Write-Host ^`^$serviceName`^ is not running, no need to stop.^; }; <# -- 3. Skip if already disabled #>; $startupType = $service.StartType <# Does not work before .NET 4.6.1 #>; if(!$startupType) { $startupType = (Get-WmiObject -Query ^Select StartMode From Win32_Service Where Name=$serviceName^ -ErrorAction Ignore).StartMode; if(!$startupType) { $startupType = (Get-WmiObject -Class Win32_Service -Property StartMode -Filter ^Name=$serviceName^ -ErrorAction Ignore).StartMode; }; }; if($startupType -eq Disabled) { Write-Host ^$serviceName is already disabled, no further action is needed^; }; <# -- 4. Disable service #>; try { Set-Service -Name ^$serviceName^ -StartupType Disabled -Confirm:$false -ErrorAction Stop; Write-Host ^Disabled `^$serviceName`^ successfully.^; } catch { Write-Error ^Could not disable `^$serviceName`^: $_^; }
:: ----------------------------------------------------------
