...
\### 1.1 Fix Inefficient Block Lookup in umc\_free()

\*\*File\*\*: `umc\_client.cpp` (lines 226-235)
...
std::unordered\_map<size\_t, size\_t> m\_ptrToBlockId; // physical offset -> block ID



// Update allocateBlockPhysical to populate mapping:

m\_ptrToBlockId\[outLocation.physicalOffset] = blockId;



// Update umc\_free to use O(1) lookup:

void umc\_free(void\* ptr, size\_t size, int device, void\* stream) {
...
&#x20;   size\_t physOffset = reinterpret\_cast<size\_t>(ptr);

&#x20;   auto it = vgm->m\_ptrToBlockId.find(physOffset);

&#x20;   if (it != vgm->m\_ptrToBlockId.end()) {

&#x20;       vgm->freeBlockPhysical(it->second);

&#x20;       vgm->m\_ptrToBlockId.erase(it);
...
std::atomic<uint32\_t> refcount{0};



// Add functions to VirtualGpuMemory:

void incrementRefcount(size\_t blockId);

void decrementRefcount(size\_t blockId);
...
&#x20;   bool initialize(size\_t numThreads = 4);

&#x20;   bool readAsync(size\_t offset, void\* buffer, size\_t size, 

&#x20;                  std::function<void(bool)> callback);

&#x20;   bool writeAsync(size\_t offset, const void\* buffer, size\_t size,
...
&#x20;   HANDLE m\_iocpHandle;

&#x20;   std::vector<std::thread> m\_workerThreads;

&#x20;   HANDLE m\_hFileMapping;

&#x20;   std::atomic<bool> m\_running;
...
&#x20;   cudaStream\_t getStream(int deviceId);

&#x20;   void returnStream(int deviceId, cudaStream\_t stream);
...
&#x20;   std::unordered\_map<int, std::queue<cudaStream\_t>> m\_streamPools;
...
cudaStream\_t stream = streamPool.getStream(deviceId);
...
\*\*File\*\*: `umc\_client.cpp` (lines 161-236)
...
&#x20;   std::vector<size\_t> blockIds;

};



std::unordered\_map<void\*, AllocationInfo> m\_allocations;



void\* umc\_alloc(size\_t size, int device, void\* stream) {

&#x20;   // Proper size alignment to block size

&#x20;   const size\_t BLOCK\_SIZE = 4 \* 1024 \* 1024;

&#x20;   size\_t numBlocks = (size + BLOCK\_SIZE - 1) / BLOCK\_SIZE;
...
&#x20;   for (size\_t i = 0; i < numBlocks; ++i) {

&#x20;       size\_t blockId = allocateNextBlock(device);

&#x20;       if (blockId == SIZE\_MAX) {
...
&#x20;   m\_allocations\[ptr] = {size, device, blockIds};
...
&#x20;   auto it = m\_allocations.find(ptr);

&#x20;   if (it != m\_allocations.end()) {

&#x20;       for (size\_t blockId : it->second.blockIds) {

&#x20;           freeBlock(blockId);

&#x20;       }

&#x20;       m\_allocations.erase(it);
...
\*\*File\*\*: Create `test\_pytorch\_integration.py`
...
&#x20;   def test\_basic\_allocation(self):
...
&#x20;       allocator = CUDAPluggableAllocator('umc\_client.dll', 'umc\_alloc', 'umc\_free')

&#x20;       torch.cuda.memory.change\_current\_allocator(allocator)
...
&#x20;   def test\_large\_model\_allocation(self):
...
&#x20;   def test\_memory\_cleanup(self):
...
&#x20;   static void logAllocation(size\_t blockId, MemoryTier tier, size\_t size);

&#x20;   static void logMigration(size\_t blockId, MemoryTier from, MemoryTier to);

&#x20;   static void logEviction(size\_t blockId, MemoryTier from);

&#x20;   

private:

&#x20;   static std::mutex s\_mutex;

&#x20;   static std::ofstream s\_logFile;
...
&#x20;   void recordAccess(size\_t blockId, uint64\_t timestamp);

&#x20;   std::vector<size\_t> predictNextBlocks(size\_t windowSize = 10);

&#x20;   void schedulePrefetch(const std::vector<size\_t>\& blockIds, MemoryTier targetTier);
...
&#x20;   std::deque<AccessPattern> m\_accessHistory;

&#x20;   std::unordered\_map<size\_t, std::vector<size\_t>> m\_transitionGraph;

&#x20;   

&#x20;   void buildTransitionGraph();

&#x20;   std::vector<size\_t> getLikelyNextBlocks(size\_t currentBlock);
...
std::pair<size\_t, std::vector<size\_t>> VirtualGpuMemory::estimateWorkingSet(int windowSize) {

&#x20;   std::unordered\_set<size\_t> workingSet;

&#x20;   

&#x20;   // Analyze recent access history

&#x20;   for (auto it = m\_accessHistory.rbegin(); it != m\_accessHistory.rend() \&\& windowSize > 0; ++it, --windowSize) {
...
&#x20;   std::vector<size\_t> blockIds(workingSet.begin(), workingSet.end());

&#x20;   

&#x20;   for (size\_t blockId : blockIds) {

&#x20;       auto it = m\_blockMap.find(blockId);

&#x20;       if (it != m\_blockMap.end()) {
...
&#x20;   size\_t blockSizeBytes = 4 \* 1024 \* 1024;

&#x20;   size\_t vramBudgetPercent = 80;

&#x20;   size\_t ramBudgetPercent = 50;

&#x20;   size\_t pagefileBudgetMB = 256;
...
\*\*File\*\*: Create `benchmark\_umc.cpp`
...
\*\*Starting Point\*\*: Begin with Phase 1.1 (fix block lookup in umc\_free) as it's the highest impact single change.
