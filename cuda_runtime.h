 ...
 *
 * This is an alternate spelling for cudaMemPrefetchBatchAsync
 * made available through function overloading.
 *
 * \sa ::cudaMemPrefetchBatchAsync
 */
template<typename T>
static __inline__ __host__ cudaError_t cudaMemPrefetchBatchAsync(
      T **dptrs, size_t *sizes, size_t count,
 ...
 * made available through function overloading.
 *
 * The ::cudaMemLocation specified by \p prefetchLocs are applicable for all the prefetches specified in the batch.
 *
 * \sa ::cudaMemPrefetchBatchAsync
 */
template<typename T>
static __inline__ __host__ cudaError_t cudaMemPrefetchBatchAsync(
      T **dptrs, size_t *sizes, size_t count, struct cudaMemLocation prefetchLocs, unsigned long long flags, cudaStream_t stream
)
{
...
/**
 * \brief Performs a batch of memory discard and prefetches asynchronously.
 *
 * This is an alternate spelling for cudaMemDiscardAndPrefetchBatchAsync
 * made available through function overloading.
 *
 * \sa ::cudaMemDiscardAndPrefetchBatchAsync
 */
template<typename T>
static __inline__ __host__ cudaError_t cudaMemDiscardAndPrefetchBatchAsync(
      T **dptrs, size_t *sizes, size_t count,
      struct cudaMemLocation *prefetchLocs, size_t *prefetchLocIdxs, size_t numPrefetchLocs,
      unsigned long long flags, cudaStream_t stream
)
 ...
 * made available through function overloading.
 *
 * The ::cudaMemLocation specified by \p prefetchLocs are applicable for all the operations in the batch.
 *
 * \sa ::cudaMemDiscardAndPrefetchBatchAsync
 */
template<typename T>
...
static __inline__ __host__ cudaError_t cudaMemcpyBatchAsync(
      T *const *dsts, U *const *srcs, const size_t *sizes, size_t count, struct cudaMemcpyAttributes *attrs,
...
template<typename T, typename U>
static __inline__ __host__ cudaError_t cudaMemcpyBatchAsync(
      T *const *dsts, U *const *srcs, const size_t *sizes, size_t count, struct cudaMemcpyAttributes attr,
       cudaStream_t hStream
)
{
  size_t attrsIdxs = 0;
...
/**
 * \brief Performs a memory copy asynchronously.
 * Allows specifying attributes for the copy.
 *
 * This is an alternate spelling for cudaMemcpyAsync
 * made available through function overloading.
 *
 * \sa ::cudaMemcpyAsync ::cudaMemcpyBatchAsync ::cudaMemcpyWithAttributesAsync
 */
template<typename T, typename U>
static __inline__ __host__ cudaError_t cudaMemcpyAsync(
      T dst, U src, const size_t size, struct cudaMemcpyAttributes *attr, cudaStream_t hStream
)
{
