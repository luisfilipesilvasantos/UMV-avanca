...
bool VirtualGpuMemory::evictLRUBlockFromGPU(int deviceId, size_t requiredSpace) {
    ...
    // Ordena pelo timestamp mais antigo (LRU)
    std::sort(blocksOnDevice.begin(), blocksOnDevice.end(), [](const auto& a, const auto& b) {
        return a.second < b.second;
    });
    ...
    for (const auto& p : blocksOnDevice) {
        size_t bid = p.first;
        auto locIt = m_blockMap.find(bid);
        if (locIt != m_blockMap.end() && locIt->second.size <= requiredSpace - freedSpace) {
            // Como removemos o const da função, já podemos chamar os métodos diretamente
            if (migrateBlockToTier(bid, MemoryTier::Ram, -1)) {
                 freedSpace += locIt->second.size;
                 m_gpuAllocatedBytes[deviceId] -= locIt->second.size;
                 if (freedSpace >= requiredSpace) {
                     std::cout << "[UMC EVICT] Successfully evicted " << freedSpace / (1024*1024) << " MB from GPU " << deviceId << " using LRU policy.\n";
                     return true;
                 }
    ...
    size_t freedSpace = 0;
    // Precisamos de remover o cast const para fazer a mutação controlada de migração interna
    auto* mutableThis = const_cast<VirtualGpuMemory*>(this);

    for (const auto& p : blocksOnDevice) {
        size_t bid = p.first;
        auto locIt = mutableThis->m_blockMap.find(bid);
        if (locIt != mutableThis->m_blockMap.end() && locIt->second.size <= requiredSpace - freedSpace) {
            if (mutableThis->migrateBlockToTier(bid, MemoryTier::Ram, -1)) {
                 freedSpace += locIt->second.size;
                 mutableThis->m_gpuAllocatedBytes[deviceId] -= locIt->second.size;
                 if (freedSpace >= requiredSpace) {
                     std::cout << "[UMC EVICT] Successfully evicted " << freedSpace / (1024*1024) << " MB from GPU " << deviceId << " using LRU policy.\n";
                     return true;
                 }
            } else if (mutableThis->migrateBlockToTier(bid, MemoryTier::Pagefile, -1)) {
                 freedSpace += locIt->second.size;
                 mutableThis->m_gpuAllocatedBytes[deviceId] -= locIt->second.size;
                 if (freedSpace >= requiredSpace) {
                     std::cout << "[UMC EVICT] Successfully evicted " << freedSpace / (1024*1024) << " MB from GPU " << deviceId << " using LRU policy (to Pagefile).\n";
...
void* VirtualGpuMemory::allocateSystemVirtual(size_t size, MemoryTier& outTier, int& outDeviceId) {
    ...
    for (size_t i = 0; i < m_gpuDevices.size(); ++i) {
        size_t gpuLimit = m_gpuLimitBytes[i];
        if (m_gpuAllocatedBytes[i] + size <= gpuLimit) {
            cudaError_t err = cudaSetDevice(static_cast<int>(i));
            if (err == cudaSuccess) {
                void* devPtr = nullptr;
                ...
                if (err == cudaSuccess) {
                    ...
                    m_gpuAllocatedBytes[i] += size;
                    outTier = MemoryTier::Vram;
                    outDeviceId = static_cast<int>(i);
                    AllocationEntry entry = { outTier, outDeviceId, size, nullptr, nullptr };
                    m_systemAllocations[devPtr] = entry;
                    return devPtr;
                } else {
                    cudaGetLastError(); 
                }
    ...
    if (m_hostAllocatedBytes + size <= m_ramLimitBytes) {
        void* hostPtr = VirtualAlloc(nullptr, size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
        if (hostPtr) {
            ...
            if (err == cudaSuccess) {
                ...
                if (err == cudaSuccess) {
                    m_hostAllocatedBytes += size;
                    outTier = MemoryTier::Ram;
                    outDeviceId = -1;
                    AllocationEntry entry = { outTier, outDeviceId, size, hostPtr, nullptr };
                    m_systemAllocations[devPtr] = entry;
                    return devPtr;
                }
    ...
    if (hFileMapping) {
        void* hostPtr = MapViewOfFile(hFileMapping, FILE_MAP_WRITE | FILE_MAP_READ, 0, 0, size);
        if (hostPtr) {
            cudaError_t err = cudaHostRegister(hostPtr, size, cudaHostRegisterMapped);
            if (err == cudaSuccess) {
                void* devPtr = nullptr;
                err = cudaHostGetDevicePointer(&devPtr, hostPtr, 0);
                if (err == cudaSuccess) {
                    m_pagefileAllocatedBytes += size; 
                    outTier = MemoryTier::Pagefile;
                    outDeviceId = -1;
                    AllocationEntry entry = { outTier, outDeviceId, size, hostPtr, hFileMapping };
                    m_systemAllocations[devPtr] = entry;
                    return devPtr;
                }
                cudaHostUnregister(hostPtr);
            }
            UnmapViewOfFile(hostPtr);
        }
...
void VirtualGpuMemory::freeSystemVirtual(void* devPtr) {
    ...
    if (it == m_systemAllocations.end()) {
        std::cerr << "[UMC WARNING] Tentativa de libertar ponteiro virtual do sistema não monitorizado: " << devPtr << std::endl;
        return;
    }
    ...
    if (entry.tier == MemoryTier::Vram) {
        ...
        if (m_gpuAllocatedBytes[entry.deviceId] >= entry.size) {
            m_gpuAllocatedBytes[entry.deviceId] -= entry.size;
        } else {
            m_gpuAllocatedBytes[entry.deviceId] = 0;
        }
        std::cout << "[UMC FREE] VRAM libertada em " << devPtr << " (GPU " << entry.deviceId << ")\n";
    } 
    else if (entry.tier == MemoryTier::Ram) {
        cudaHostUnregister(entry.hostPtr);
        VirtualFree(entry.hostPtr, 0, MEM_RELEASE);
        
        if (m_hostAllocatedBytes >= entry.size) {
            m_hostAllocatedBytes -= entry.size;
        } else {
            m_hostAllocatedBytes = 0;
        }
        std::cout << "[UMC FREE] System RAM libertada em " << devPtr << "\n";
    } 
    else if (entry.tier == MemoryTier::Pagefile) {
        cudaHostUnregister(entry.hostPtr);
        UnmapViewOfFile(entry.hostPtr);
        ...
        if (m_pagefileAllocatedBytes >= entry.size) {
            m_pagefileAllocatedBytes -= entry.size;
        } else {
            m_pagefileAllocatedBytes = 0;
        }
        std::cout << "[UMC FREE] Mapeamento de Pagefile libertado em " << devPtr << "\n";
    }
...
bool VirtualGpuMemory::isSystemAllocation(void* devPtr, BlockLocation& outLoc) const {
    ...
    auto it = m_systemAllocations.find(devPtr);
    if (it != m_systemAllocations.end()) {
        outLoc.tier = it->second.tier;
        outLoc.deviceId = it->second.deviceId;
        outLoc.size = it->second.size;
        outLoc.physicalOffset = reinterpret_cast<size_t>(devPtr);
        return true;
    }
    return false;
}