#!/usr/bin/env python3
"""Extrai funções específicas do VirtualGpuMemory.cpp usando parsing simples."""

def find_function(content, func_name):
    """Encontra a definição da função e seu corpo (entre chaves balanceadas)."""
    # Procura pelo início da assinatura
    idx = content.find(func_name)
    if idx == -1:
        return None
    
    # Encontra o '{' inicial (pode haver parâmetros entre parênteses antes)
    paren_depth = 0
    brace_start = -1
    ...
    # Encontra o '}' final (balanceando chaves)
    depth = 1
...
def main():
    ...
    for func_sig, name in functions:
        ...
        print(f"\n=== {name} ===")
        if result:
            # Limita a 2500 chars para legibilidade
