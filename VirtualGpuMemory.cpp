...
bool VirtualGpuMemory::initialize() {
    ...
    // Inicializar as streams CUDA de forma segura assente no número de GPUs detetadas
    {
        ...
        for (size_t i = 0; i < m_gpuDevices.size(); ++i) {
            ...
            if (m_streams.find(static_cast<int>(i)) == m_streams.end()) {
                cudaStreamCreate(&m_streams[static_cast<int>(i)]);
            }
        }
    }
...
void VirtualGpuMemory::logResidencyChange(size_t blockId, MemoryTier newTier, int newDeviceId) {
    ...
    auto& history = m_blockResidencyHistory[blockId];
    history.push_back(entry);
    if (history.size() > static_cast<size_t>(m_historyWindowSize)) {
...
bool VirtualGpuMemory::freeBlockPhysical(size_t blockId) {
    std::lock_guard<std::recursive_mutex> lock(m_mutex);
    ...
    switch (loc.tier) {
        case MemoryTier::Vram:
            if (loc.deviceId >= 0 && static_cast<size_t>(loc.deviceId) < m_gpuAllocatedBytes.size()) {
                m_gpuAllocatedBytes[loc.deviceId] -= loc.size;
            }
            break;
        case MemoryTier::Ram:
        ...
        case MemoryTier::Pagefile:
            // Na versão mock, apenas removemos da page table
            break;
    }
    
    m_blockMap.erase(it);
    m_blockData.erase(blockId);
    return true;
}
