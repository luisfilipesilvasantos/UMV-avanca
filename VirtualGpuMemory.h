...
// Always use real CUDA runtime API - mocks were causing issues with defines and linking
...
public:
    ...
    // Membros de histórico e timestamps para thrashing detection
    std::deque<std::pair<uint64_t, size_t>> m_accessHistory;
    std::vector<ResidencyHistoryEntry> m_residencyHistory;
    std::unordered_map<size_t, uint64_t> m_accessTimestamps;
    std::unordered_map<size_t, std::vector<ResidencyHistoryEntry>> m_blockResidencyHistory;
};
