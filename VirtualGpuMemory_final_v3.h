...
private:
    ...
    // Membros privados (base)
    size_t m_blockSizeBytes;
    std::unordered_map<size_t, BlockLocation> m_blocks;  // blockId -> location (usado no VirtualGpuMemory.cpp atual)
    ...
    mutable std::recursive_mutex m_mutex;  // Mutex principal para operações de memória (mutable para permitir lock em métodos const)
    std::mutex m_streamMutex;              // Mutex para streams CUDA
    std::vector<cudaStream_t> m_streams;   // deviceId -> stream (usado no patched.cpp: resize() e size())
    ...
    std::unordered_map<size_t, std::deque<ResidencyHistoryEntry>> m_residencyHistory; // histórico por bloco
    std::vector<size_t> m_gpuLimitBytes;  // limite por GPU (do patched.cpp)

    // Alocações 'system virtual' (usadas no VirtualGpuMemory.cpp atual)
